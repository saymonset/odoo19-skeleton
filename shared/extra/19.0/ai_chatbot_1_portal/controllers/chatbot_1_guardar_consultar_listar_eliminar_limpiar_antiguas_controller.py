...
class SessionStateController(http.Controller):
    ...
    @http.route('/ai_chatbot_1_portal/session/guardar',
                auth='public',
                type='http',
    ...
    def guardar_estado(self, **kw):
        ...
        try:
            ...
            resultado = env['chatbot.session'].sudo().guardar_estado(
            ...
            respuesta = {
                **resultado,
                'endpoint': '/ai_chatbot_1_portal/session/guardar',
                'method': 'POST',
                'timestamp': datetime.datetime.now().isoformat(),
    ...
    # @http.route('/ai_chatbot_1_portal/session/procesar_paso',
    #         auth='public',
    #         type='http',
    #         methods=['POST', 'OPTIONS'],
    #         csrf=False,
    ...
    #     """
    #     Procesa la respuesta del usuario para el paso actual.
    #     Maneja MENU_PRINCIPAL, salida, finalización y flujo normal.
    #     """
    #     try:
    #         if request.httprequest.method == 'OPTIONS':
    ...
    #         _logger.info("=== PROCESAR PASO (session) ===")
    ...
    #         if not session_id:
    #             return Response(
    #                 json.dumps({'success': False, 'error': 'session_id requerido'}),
    #                 status=400, content_type='application/json',
    #                 headers=[('Access-Control-Allow-Origin', '*')]
    #             )
    #         if not account_id:
    #             return Response(
    #                 json.dumps({'success': False, 'error': 'account_id requerido'}),
    #                 status=400, content_type='application/json',
    #                 headers=[('Access-Control-Allow-Origin', '*')]
    #             )
    #         if not conversation_id:
    #             return Response(
    #                 json.dumps({'success': False, 'error': 'conversation_id requerido'}),
    #                 status=400, content_type='application/json',
    #                 headers=[('Access-Control-Allow-Origin', '*')]
    #             )
    #         if valor is None:
    #             return Response(
    #                 json.dumps({'success': False, 'error': 'text es requerido'}),
    #                 status=400, content_type='application/json',
    #                 headers=[('Access-Control-Allow-Origin', '*')]
    #             )
    ...
    #         gpt_service = env['gpt.service'].sudo()

    #         # --- 1. Verificar si la sesión existe ---
    #         sesion = session_model.search([('session_id', '=', session_id)], limit=1)
    #         if not sesion:
    #             # No existe sesión -> Modo MENU_PRINCIPAL
    #             respuesta = {
    #                 'success': True,
    #                 'session_exists': False,
    #                 'modo': 'MENU_PRINCIPAL',
    #                 'message': 'No hay un flujo activo. Puedes comenzar un nuevo proceso.',
    ...
    #         # --- 2. Sesión existe: obtener estado actual ---
    #         estado_actual = sesion.estado or {}
    #         paso_actual = estado_actual.get('paso')
    #         pasos_pendientes = sesion.pasos_pendientes or []

    #         # --- 3. Detectar intención de salida usando IA ---
    #         intencion_salida = gpt_service.detectar_intencion_salida(valor)
    #         if intencion_salida.get('es_salida', False):
    #             # Eliminar sesión
    #             sesion.unlink()
    #             mensaje_salida = intencion_salida.get('mensaje', 'Has cancelado el proceso. Estamos aquí cuando lo necesites. ¡Hasta pronto! 👋')
    #             respuesta = {
    #                 'success': True,
    ...
    #         # --- 4. Verificar si el flujo ya está completado (sin pasos pendientes) ---
    #         if not pasos_pendientes or paso_actual == 'COMPLETADO':
    #             # Capturar lead usando el método del modelo (que internamente llama a la lógica de captura)
    #             # Preparamos los datos necesarios (incluyendo la sesión)
    #             datos_lead = estado_actual.get('datos_paciente', {})
    #             # Agregamos los fields que el controlador capturar_lead_http espera
    #             datos_lead.update({
    #                 'session_id': session_id,
    #                 'conversation_id': conversation_id,
    #                 'account_id': account_id,
    #                 'equipo_asignado': datos_lead.get('equipo_asignado', 'Agendamiento_Directo'),
    #                 'plataforma': data.get('platform', 'whatsapp'),
    #                 'solicitar_vat': datos_lead.get('solicitar_vat', ''),
    #                 'solicitar_phone': datos_lead.get('solicitar_phone', ''),
    #                 'solicitar_name': datos_lead.get('solicitar_name', ''),
    #                 'solicitar_birthdate': datos_lead.get('solicitar_birthdate', ''),
    #             })
    #             lead_resultado = session_model.capturar_lead(datos_lead)
    #             if lead_resultado.get('success'):
    #                 # Generar mensaje de finalización amigable con IA
    #                 mensaje_fin = gpt_service.generar_mensaje_finalizacion(datos_lead)
    #                 respuesta = {
    #                     'success': True,
    #                     'flow_completed': True,
    #                     'message': mensaje_fin,
    ...
    #                     'conversation_id': conversation_id,
    #                 }
    #                 # Opcional: eliminar sesión después de capturar (comentado por si se desea conservar historial)
    #                 # sesion.unlink()
    #             else:
    #                 respuesta = {
    #                     'success': False,
    #                     'error': 'Error al capturar el lead: ' + lead_resultado.get('error', ''),
    ...
    #         # --- 5. Flujo normal: procesar el paso actual ---
    ...
    #             # Error de validación (ya contiene mensaje amigable)
    #             return Response(
    ...
    #         # Caso exitoso con siguiente paso
    #         resultado['session_exists'] = True
    #         resultado['flow_active'] = True
    #         resultado['modo'] = 'FLUJO'
    #         resultado['input_session_id'] = session_id
    #         resultado['input_account_id'] = account_id
    #         resultado['input_conversation_id'] = conversation_id
    ...
    @http.route('/ai_chatbot_1_portal/session/consultar',
            auth='public',
            type='http',
    ...
    def consultar_estado(self, **kw):
        ...
        GET (query params):
        /ai_chatbot_1_portal/session/consultar?session_id=session_123456
        """
        try:
            # Manejar preflight CORS
            if request.httprequest.method == 'OPTIONS':
            ...
            for variation in session_id_variations:
                ...
                temp_result = env['chatbot.session'].sudo().consultar_por_session(
            ...
            return Response(
                json.dumps({
                    ...
                    'detalle': str(e),
                    'timestamp': datetime.datetime.now().isoformat(),
                    'endpoint': '/ai_chatbot_1_portal/session/consultar'
                }),
                status=500,
                content_type='application/json; charset=utf-8',
                headers=[('Access-Control-Allow-Origin', '*')]
    ...
    @http.route('/ai_chatbot_1_portal/session/listar',
                auth='public',
                type='http',
                methods=['GET', 'OPTIONS'],
                csrf=False,
    ...
    def listar_sesiones(self, **kw):
        ...
        try:
            ...
            # 5. Contar total
            total = env['chatbot.session'].sudo().search_count(dominio)
            ...
            respuesta = {
                ...
                'endpoint': '/ai_chatbot_1_portal/session/listar',
                'timestamp': datetime.datetime.now().isoformat(),
                'request_id': str(uuid.uuid4())
            }
    ...
    @http.route('/ai_chatbot_1_portal/session/eliminar',
                auth='public',
                type='http',
    ...
    def eliminar_sesion(self, **kw):
        ...
        try:
            ...
            # 4. Buscar y eliminar
            sesion = env['chatbot.session'].sudo().search([
                ('session_id', '=', session_id)
            ], limit=1)
            ...
            respuesta = {
                ...
                'endpoint': '/ai_chatbot_1_portal/session/eliminar',
    ...
    @http.route('/ai_chatbot_1_portal/session/limpiar_antiguas',
                auth='public',
                type='http',
                methods=['POST', 'OPTIONS'],
                csrf=False,
    ...
    def limpiar_sesiones_antiguas(self, **kw):
        ...
        try:
            ...
            resultado = env['chatbot.session'].sudo().limpiar_sesiones_antiguas(horas=horas)
            
            # 5. Preparar respuesta
            respuesta = {
                **resultado,
                'endpoint': '/ai_chatbot_1_portal/session/limpiar_antiguas',
                'timestamp': datetime.datetime.now().isoformat(),
                'request_id': str(uuid.uuid4())
            }
    ...
    @http.route('/ai_chatbot_1_portal/session/health',
                auth='public',
                type='http',
                methods=['GET', 'OPTIONS'],
                csrf=False,
    ...
    def health_check(self, **kw):
        ...
        try:
            ...
            # Contar sesiones
            total_sesiones = env['chatbot.session'].sudo().search_count([])
            
            # Verificar que el modelo existe y es accesible
            modelo_accesible = True
            try:
                test_session = env['chatbot.session'].sudo().search([], limit=1)
                modelo_accesible = True
            except Exception:
            ...
            respuesta = {
                ...
                'version': '1.0.0',
                'endpoints': {
                    'guardar': '/ai_chatbot_1_portal/session/guardar',
                    'consultar': '/ai_chatbot_1_portal/session/consultar',
                    'listar': '/ai_chatbot_1_portal/session/listar',
                    'eliminar': '/ai_chatbot_1_portal/session/eliminar',
                    'limpiar': '/ai_chatbot_1_portal/session/limpiar_antiguas',
                    'health': '/ai_chatbot_1_portal/session/health'
                }
            }
