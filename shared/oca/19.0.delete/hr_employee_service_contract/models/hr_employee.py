...
class HrEmployee(models.Model):
    ...
    @api.depends("version_ids", "version_ids.contract_date_start")
    def _compute_first_contract_id(self):
        Contract = self.env["hr.version"]
        for employee in self:
            employee.first_contract_id = Contract.search(
                employee._get_contract_filter(), order="contract_date_start asc", limit=1
            )

    @api.depends("version_ids", "version_ids.contract_date_end")
    def _compute_last_contract_id(self):
        Contract = self.env["hr.version"]
        for employee in self:
            employee.last_contract_id = Contract.search(
                employee._get_contract_filter(), order="contract_date_end desc", limit=1
            )
